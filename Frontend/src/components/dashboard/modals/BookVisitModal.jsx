...
export default function BookVisitModal({ isOpen, onClose, onBook }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-surface-900/50 backdrop-blur-sm transition-opacity">
      <div className="relative w-full max-w-sm rounded-2xl bg-white shadow-xl dark:bg-surface-800 p-6 animate-in zoom-in-95 duration-200">
        <button onClick={onClose} className="absolute right-4 top-4 rounded-full p-2 text-surface-400 transition-colors hover:bg-surface-100 dark:hover:bg-surface-700">
          <X size={20} />
        </button>

        <h2 className="text-xl font-bold text-surface-900 dark:text-white mb-6">Book Visit</h2>
