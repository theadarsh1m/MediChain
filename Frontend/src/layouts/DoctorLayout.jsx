...
import { getInitials } from "../lib/patient"; // Reusing getInitials
...
export default function DoctorLayout() {
  ...
  const { user: doctor, logout: handleLogout } = useAuth();
          ...
          <div className="absolute left-0 top-0 flex h-full w-[88vw] max-w-xs flex-col border-r border-slate-200 bg-white px-4 py-5 shadow-2xl transition-colors dark:border-slate-800 dark:bg-navbar-bg">
            <div className="mb-6 flex items-center justify-between gap-3">
              <Link to="/doctor/dashboard" className="flex items-center gap-3">
                <img src={mediChainLogo} alt="Logo" className="h-11 w-11 rounded-2xl border border-slate-200 bg-white p-2 object-contain shadow-sm dark:border-slate-700 dark:bg-slate-900" />
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.2em] text-blue-500 dark:text-emerald-300">MediChain</p>
                  <p className="text-sm text-slate-500 dark:text-slate-400">Doctor Portal</p>
                </div>
              </Link>
              <button type="button" onClick={() => setMobileNavOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-200 text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                <X size={18} />
              </button>
            </div>
          ...
          <div className="flex h-full flex-col">
            <div>
              <Link to="/doctor/dashboard" className="mb-8 flex items-center gap-3 px-2 transition-opacity hover:opacity-80">
                <img src={mediChainLogo} alt="Logo" className="h-12 w-12 rounded-2xl border border-slate-200 bg-white p-2 object-contain shadow-sm dark:border-slate-700 dark:bg-slate-900" />
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.2em] text-blue-500 dark:text-emerald-300">MediChain</p>
                  <p className="text-sm text-slate-500 dark:text-slate-400">Doctor Portal</p>
                </div>
            ...
            <div className="flex flex-col gap-3 px-4 py-1 sm:px-6 lg:px-8">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 items-start gap-3 lg:items-center">
                  <button type="button" onClick={() => setMobileNavOpen(true)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-slate-200 text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800 lg:hidden">
                    <Menu size={18} />
                  </button>

                  <div className="min-w-0">
                    <p className="text-sm text-slate-500 dark:text-slate-400">Doctor Portal</p>
                    <h1 className="mt-1 truncate text-2xl font-semibold text-slate-900 dark:text-slate-100">{currentItem.label}</h1>
                  </div>
                </div>

                <div className="flex items-center gap-3 sm:justify-end">
                  <Classic duration={750} toggled={isDark} onClick={toggleTheme} className="shrink-0 text-3xl text-slate-600 transition-all dark:text-yellow-400" />
                </div>
              </div>
            </div>
          </header>
